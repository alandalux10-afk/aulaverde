...
ipcMain.handle('guardar-configuracion', (event, datos) => {
  try {
    ...
    const { guardarDB } = require('./src/js/database')
    db.run(
      'UPDATE CONFIGURACION SET nombre_tienda=?, razon_social=?, nif_vendedor=?, direccion=?, telefono=?, email=?, impresora_ticket=?, impresora_factura=?, api_key_anthropic=?, puntos_euros_por_punto=?, puntos_valor_canje=? WHERE id_configuracion=1',
      [
        datos.nombre_tienda,
        datos.razon_social,
        datos.nif_vendedor,
...
ipcMain.handle('exportar-listado-facturas', async (event, filtros) => {
  try {
    ...
    // Obtener todas las compras del período con sus líneas
    ...
    // Para cada compra, obtener las líneas agrupadas por tipo de IVA
    const filas = []
    ...
    compras.forEach(compra => {
      ...
      // Inicializar valores por tipo de IVA
      let base4 = 0, iva4 = 0, recargo4 = 0
      let base10 = 0, iva10 = 0, recargo10 = 0
      let base21 = 0, iva21 = 0, recargo21 = 0
      let base0 = 0, iva0 = 0, recargo0 = 0
      let baseTotal = 0
      ...
      if (lineasResult.length && lineasResult[0].values.length) {
        lineasResult[0].values.forEach(row => {
          ...
          if (pct === 4) {
            base4 = base; iva4 = iva
            recargo4 = tieneRecargo ? Number((base * 0.005).toFixed(2)) : 0
          } else if (pct === 10) {
            base10 = base; iva10 = iva
            recargo10 = tieneRecargo ? Number((base * 0.014).toFixed(2)) : 0
          } else if (pct === 21) {
            base21 = base; iva21 = iva
            recargo21 = tieneRecargo ? Number((base * 0.052).toFixed(2)) : 0
          } else if (pct === 0) {
            base0 = base; iva0 = iva
            recargo0 = 0
          }
      ...
      filas.push({
        'Nº Factura': compra.numero_factura,
        'Fecha': compra.fecha,
        'Proveedor': compra.nombre_proveedor,
        'Base': Number(baseTotal.toFixed(2)),
        'Base IVA 4%': base4 || '',
        'IVA 4% Superreducido': iva4 || '',
        'Rec. Equiv. 4%': recargo4 || '',
        'Base IVA 10%': base10 || '',
        'IVA 10% Reducido': iva10 || '',
        'Rec. Equiv. 10%': recargo10 || '',
        'Base IVA 21%': base21 || '',
        'IVA 21% General': iva21 || '',
        'Rec. Equiv. 21%': recargo21 || '',
        'Base IVA 0%': base0 || '',
        'IVA 0% Exento': iva0 || '',
        'Rec. Equiv. 0%': recargo0 || '',
...
ipcMain.handle('crear-cliente', (event, datos) => {
  try {
    ...
    const { guardarDB } = require('./src/js/database')
    db.run(
      'INSERT INTO CLIENTES (nombre, telefono, email, fecha_nacimiento, direccion, nif, notas, activo, descuento) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)',
      [
        datos.nombre,
        datos.telefono,
        datos.email,
...
ipcMain.handle('editar-cliente', (event, idCliente, datos) => {
  try {
    ...
    const { guardarDB } = require('./src/js/database')
    db.run(
      'UPDATE CLIENTES SET nombre=?, telefono=?, email=?, fecha_nacimiento=?, direccion=?, nif=?, notas=?, descuento=? WHERE id_cliente=?',
      [
        datos.nombre,
        datos.telefono,
        datos.email,
...
ipcMain.handle('obtener-historial-cliente', (event, idCliente) => {
  const db = getDB()
  const result = db.exec(`
    SELECT v.id_venta, v.numero_documento, v.fecha, v.hora, v.tipo_documento,
...
ipcMain.handle('importar-clientes-excel', async (event) => {
  try {
    ...
    filas.forEach(fila => {
      ...
      // Evitar duplicados por teléfono si existe, si no por nombre
      let existe
      if (telefono) {
      ...
      db.run(
        'INSERT INTO CLIENTES (nombre, telefono, email, direccion, nif, notas, activo, descuento) VALUES (?, ?, ?, ?, ?, ?, 1, 0)',
...
ipcMain.handle('obtener-alertas-crm', () => {
  ...
  // Cumpleaños en los próximos 7 días (comparando mes y día)
  const hoy = new Date()
  const cumpleanos = []
  ...
  if (todosResult.length && todosResult[0].values.length) {
    todosResult[0].values.forEach(row => {
      ...
      const cumpleEsteAno = new Date(hoy.getFullYear(), mesNac - 1, diaNac)
  ...
  // Clientes sin compras en los últimos 90 días (pero que sí tienen alguna compra registrada)
  ...
  const inactivosResult = db.exec(`
    ...
    HAVING MAX(v.fecha) < '${fechaLimiteStr}'
