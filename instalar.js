...
function importarProductos() {
  ...
  for (const p of productos) {
    try {
      ...
      db.run(
        'INSERT INTO PRODUCTOS (codigo, codigo_barras, nombre, familia, tipo_venta, precio_venta, id_iva, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [codigo, p.codigo_barras || '', nombre, p.familia || '', tipo_venta, precio_venta, id_iva, activo]
      )
      importados++
    } catch (e) {
      errores++
...
fs2.writeFileSync('src/js/importar-productos.js', require('fs').readFileSync(__filename, 'utf8').split('const fs2')[0])